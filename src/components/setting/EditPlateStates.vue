...
export default {
  ...
  methods: {
    Modify () {
      ...
      if (app.description !== '') {
        const dateOrderPlate = {
          description: app.description
        }
        const PLAESTATE_URL = process.env.API_URL + '/platestates/' + id
        axios.put(PLAESTATE_URL, dateOrderPlate, config).then(function (response) {
          if (response.status === 200) {
            ...
            app.$router.push('/main/list-order-plate/')
          } else {
            Notify.create({
    ...
    viewStatePlate () {
      ...
      const STATEORDER_URL = process.env.API_URL + '/platestates/' + id
      axios.get(STATEORDER_URL, config).then(function (response) {
        app.description = response.data.PlateStates.description
      })
    }
  }
}
