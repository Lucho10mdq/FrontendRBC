  ...
  methods: {
    ModifyWaiter () {
      ...
      if ((app.name !== '') && (app.dni !== '') && (app.doc !== '') && (app.phone !== '') && (app.email !== '') && (app.code !== '')) {
        ...
        const CUSTOMERS_URL = process.env.API_URL + '/customers/' + id
        axios.put(CUSTOMERS_URL, dateWaiters, config).then(function (response) {
          if (response.status === 200) {
            Notify.create({
              message: 'Se modifico cliente correctamente',
              type: 'positive',
              position: 'top-right'
            })
            app.$router.push('/main/list-customer/')
          } else {
            Notify.create({
    ...
    viewWaiter () {
      ...
      const CUSTOMERS_URL = process.env.API_URL + '/customers/' + id
      axios.get(CUSTOMERS_URL, config).then(function (response) {
